...
func processStack(rawStack []byte) ([]byte, error) {
	...
	for _, bucket := range buckets {
		for _, line := range bucket.Signature.Stack.Calls {
			if l := len(fmt.Sprintf("%s/%s:%d", stripDiginfraImportPrefix(line.ImportPath), line.SrcName, line.Line)); l > srcLen {
				srcLen = l
			}
		}
	}
	...
	for _, bucket := range buckets {
		...
		for _, line := range bucket.Stack.Calls {
			_, err := fmt.Fprintf(w,
				"   %-*s %s()\n",
				srcLen,
				fmt.Sprintf("%s/%s:%d", stripDiginfraImportPrefix(line.ImportPath), line.SrcName, line.Line),
				line.Func.Name)
			if err != nil {
				return []byte{}, err
			}
...
func stripDiginfraImportPrefix(s string) string {
	return strings.TrimPrefix(s, "github.com/diginfra/diginfra/")
}
