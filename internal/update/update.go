...
func CheckForUpdate(ctx *config.RunContext) (*Info, error) {
	...
	var cmd string
	if isBrew {
		cmd = "$ brew upgrade diginfra"
	} else {
		cmd = "Go to https://www.diginfra.khulnasoft.com/docs/update for instructions"
		if runtime.GOOS == "linux" || runtime.GOOS == "darwin" {
			cmd = "$ curl -fsSL https://raw.githubusercontent.com/diginfra/diginfra/master/scripts/install.sh | sh"
		}
	}
...
func isBrewInstall() (bool, error) {
	...
	if err != nil {
		return false, errors.Wrap(err, "error finding diginfra executable")
	}

	path, err := filepath.EvalSymlinks(exe)
	if err != nil {
		return false, errors.Wrap(err, "error evaluating diginfra executable symlink")
	}

	brewPrefixCmd := exec.Command("brew", "--prefix", "diginfra")
	...
	if err != nil {
		return false, errors.Wrap(err, "error running 'brew --prefix diginfra'")
	}
	...
	brewDiginfraPath := filepath.Join(brewPrefixPath, "bin", "diginfra")

	return path == brewDiginfraPath, nil
}
...
func getLatestBrewVersion() (string, error) {
	...
	resp, err := http.Get("https://formulae.brew.sh/api/formula/diginfra.json")
	if err != nil {
		return "", err
	}
...
func getLatestGitHubVersion() (string, error) {
	...
	resp, err := http.Get("https://api.github.com/repos/diginfra/diginfra/releases/latest")
	if err != nil {
		return "", err
	}
	defer resp.Body.Close()
