...
import (
	...
	"github.com/diginfra/diginfra/internal/providers/cloudformation"
	"github.com/diginfra/diginfra/internal/providers/terraform"
	...
	if maxDepth > 0 {
		...
		if err == nil {
			for _, entry := range entries {
				name := entry.Name()
				if entry.IsDir() && name != config.DiginfraDir && name != ".terraform" {
