...
func NewAutoscalingGroup(d *schema.ResourceData) schema.CoreResource {
	...
	if !d.IsEmpty("desired_capacity") {
		...
		if instanceCount == 0 {
			logging.Logger.Debug().Msgf("Using instance count 1 for %s since no desired_capacity or non-zero min_size is set. To override this set the instance_count attribute for this resource in the Diginfra usage file.", a.Address)
			instanceCount = 1
		}
	}
