...
import (
	...
	"github.com/diginfra/diginfra/internal/hcl"
	"github.com/diginfra/diginfra/internal/hcl/modules"
...
var (
	...
	moduleCacheRegex = regexp.MustCompile(`(?:.+)?\.diginfra/terraform_modules/[^/]+/(.+)`)
)
...
// The PlanJSONProvider uses this shallow representation to actually load Diginfra resources.
...
func (p *HCLProvider) LoadPlanJSON() HCLProject {
	...
	if module.Error == nil {
		...
		if os.Getenv("DIGINFRA_JSON_DUMP") == "true" {
			targetPath := fmt.Sprintf("%s-out.json", strings.ReplaceAll(module.Module.ModulePath, "/", "-"))
			if os.Getenv("DIGINFRA_JSON_DUMP_PATH") != "" {
				targetPath = filepath.Join(os.Getenv("DIGINFRA_JSON_DUMP_PATH"), targetPath)
			}
...
func (p *HCLProvider) newPlanSchema() {
	p.schema = &PlanSchema{
		...
		DiginfraResourceChanges: []ResourceChangesJSON{},
...
func (p *HCLProvider) modulesToPlanJSON(rootModule *hcl.Module) ([]byte, error) {
	...
	if rootModule.ProviderConstraints != nil {
		p.schema.DiginfraProviderConstraints = *rootModule.ProviderConstraints
	}
...
func (p *HCLProvider) marshalModule(module *hcl.Module) ModuleOut {
	...
	for _, block := range module.Blocks {
		if block.Type() == "resource" {
			...
			p.schema.DiginfraResourceChanges = append(p.schema.DiginfraResourceChanges, out.Changes)
		}
	}
...
type ResourceJSON struct {
	...
	DiginfraMetadata map[string]interface{} `json:"diginfra_metadata"`
}
...
type PlanSchema struct {
	...
	// DiginfraResourceChanges is a flattened list of resource changes for the plan, this is in the format of the Terraform
	// plan JSON output, but we omit adding it as the supported `resource_changes` key as this will cause plan inconsistencies.
	// We copy this `diginfra_resource_changes` key at a later date to `resource_changes` before sending to the Policy API.
	// This means that we can evaluate the Rego ruleset on the known Terraform plan JSON structure.
	DiginfraResourceChanges     []ResourceChangesJSON   `json:"diginfra_resource_changes"`
	DiginfraProviderConstraints hcl.ProviderConstraints `json:"diginfra_provider_constraints"`
}
