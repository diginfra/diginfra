...
var cacheFileName = ".diginfra-cache"
...
func WritePlanCache(p *DirProvider, planJSON []byte) {
	...
	// create the .diginfra dir if it doesn't already exist
	if _, err := os.Stat(cacheDir); err != nil {
		if os.IsNotExist(err) {
			err := os.MkdirAll(cacheDir, 0700)
			if err != nil {
				logging.Logger.Debug().Msgf("Couldn't create %v directory: %v", config.DiginfraDir, err)
				return
			}
		}
	}
...
func calcCacheDir(p *DirProvider) string {
	...
	if dataDir != (path.Join(p.Path, ".terraform")) {
		...
		return path.Join(dataDir, config.DiginfraDir)
	}

	return path.Join(p.Path, config.DiginfraDir)
}
...
func calcTFDataDate(path string, maxDepth int) time.Time {
	...
	if err == nil {
		for _, entry := range entries {
			if entry.Name() == config.DiginfraDir {
