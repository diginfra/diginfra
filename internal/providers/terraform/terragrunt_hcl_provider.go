...
import (
	...
	infSync "github.com/diginfra/diginfra/internal/sync"
...
func getEnvVars(ctx *config.ProjectContext) map[string]string {
	...
	if v, ok := os.LookupEnv("DIGINFRA_SAFE_ENVS"); ok {
		filterSafe = true
...
func (p *TerragruntHCLProvider) prepWorkingDirs() ([]*terragruntWorkingDirInfo, error) {
	...
	terragruntCacheDir := filepath.Join(config.DiginfraDir, ".terragrunt-cache")
	...
	if err != nil {
		return nil, clierror.NewCLIError(
			fmt.Errorf(
				...
				err.Error(),
				fmt.Sprintf("For a list of known issues and workarounds, see: %s", ui.LinkString("https://diginfra.khulnasoft.com/docs/features/terragrunt/")),
			),
...
// downloadSource overrides the Terragrunt download source functionality to use the Diginfra packageFetcher
...
func loadSourceOnce(sourceURL string, opts *tgoptions.TerragruntOptions, terragruntConfig *tgconfig.TerragruntConfig, packageFetcher *modules.PackageFetcher, logger zerolog.Logger) (string, error) {
	...
	sparseCheckoutEnabled := os.Getenv("DIGINFRA_SPARSE_CHECKOUT") == "true"
