...
func (p *DirProvider) checks() error {
	...
	if err != nil {
		...
		msg += "1. Set a custom Terraform binary using the environment variable DIGINFRA_TERRAFORM_BINARY.\n\n"
		msg += fmt.Sprintf("2. Set --path to a Terraform plan JSON file. See %s for how to generate this.", ui.LinkString("https://diginfra.khulnasoft.com/troubleshoot"))
...
func checkTerraformVersion(v string, fullV string) error {
	...
	if strings.HasPrefix(fullV, "Terraform ") && semver.Compare(v, minTerraformVer) < 0 {
		return fmt.Errorf("Terraform %s is not supported. Please use Terraform version >= %s. Update it or set the environment variable DIGINFRA_TERRAFORM_BINARY.", v, minTerraformVer) // nolint
	}

	if strings.HasPrefix(fullV, "terragrunt") && semver.Compare(v, minTerragruntVer) < 0 {
		return fmt.Errorf("Terragrunt %s is not supported. Please use Terragrunt version >= %s. Update it or set the environment variable DIGINFRA_TERRAFORM_BINARY.", v, minTerragruntVer) // nolint
	}
...
func (p *DirProvider) buildTerraformErr(err error, isInit bool) error {
	...
	if strings.HasPrefix(stderr, "Error: Failed to select workspace") {
		...
		msg += "\n\nYou have two options:\n"
		msg += "1. Run `terraform login` or set the DIGINFRA_TERRAFORM_CLOUD_TOKEN environment variable.'\n\n"
		msg += fmt.Sprintf("2. Set --path to a Terraform plan JSON file. See %s for how to generate this.", ui.LinkString("https://diginfra.khulnasoft.com/troubleshoot"))
		...
		msg += fmt.Sprintf("\n\nSee %s for how to generate multiple Terraform plan JSON files for your Terragrunt project, and use them with Diginfra.", ui.LinkString("https://diginfra.khulnasoft.com/troubleshoot"))
	} else if isInit {
		msg += fmt.Sprintf("\n\nTry using --terraform-init-flags to pass any required Terraform init flags, or skip Terraform init entirely and set the --path to a Terraform plan JSON file. See %s for how to generate this.", ui.LinkString("https://diginfra.khulnasoft.com/troubleshoot"))
	} else if strings.HasPrefix(stderr, "Error: Unsupported Terraform Core version") {
		msg += "\n\nUpdate Terraform to the required version or set a custom Terraform binary using the environment variable DIGINFRA_TERRAFORM_BINARY."
	} else {
		msg += fmt.Sprintf("\n\nTry setting the --path to a Terraform plan JSON file. See %s for how to generate this.", ui.LinkString("https://diginfra.khulnasoft.com/troubleshoot"))
	}
