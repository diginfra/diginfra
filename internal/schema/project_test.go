...
func TestGenerateProjectName(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			args: args{
				...
				remote:   vcs.Remote{Name: "diginfra/remote-repo"},
			},
			want: "diginfra/remote-repo",
		},
		{
			...
			want: "diginfra/remote-repo/sub/path",
		},
		{
	...
	t.Run("repo without remote metadata, Diginfra Cloud is enabled", func(t *testing.T) {
		testArgs := args{
			metadata:         &ProjectMetadata{Path: "path/to/repo/test-repo"},
			remote:           vcs.Remote{},
	...
	tests := []test{
		{
			...
			createProject: func(t *testing.T) (input args, cleanup func()) {
				t.Setenv("DIGINFRA_VCS_REPOSITORY_URL", "test")
