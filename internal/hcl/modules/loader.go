...
// .diginfra/terraform_modules directory. We could implement a global cache in the future, but for now have decided
// to go with the same approach as Terraform.
type ModuleLoader struct {
	// cachePath is the path to the directory that Diginfra will download modules to.
	// This is normally the top level directory of a multi-project environment, where the
	// Diginfra config file resides or project auto-detection starts from.
	cachePath string
	cache     *Cache
...
func (m *ModuleLoader) manifestFilePath(projectPath string) string {
	...
	return filepath.Join(m.cachePath, ".diginfra/terraform_modules/", fmt.Sprintf("manifest-%x.json", sum))
}
	...
	if errors.Is(err, os.ErrNotExist) {
		...
		if err == nil {
			...
			if err == nil {
				...
				// we'll look up the modules based on the cache path at the Diginfra root (where the diginfra.yml
...
func RecursivelyAddDirsToSparseCheckout(repoRoot string, sourceURL string, packageFetcher *PackageFetcher, existingDirs []string, dirs []string, mu *sync.Mutex, logger zerolog.Logger, depth int) error {
	...
	tmpDir, err := os.MkdirTemp(os.TempDir(), "diginfra-sparse-checkout")
	if err != nil {
		return err
	}
...
func (m *ModuleLoader) loadRemoteModule(key string, source string) (*ManifestModule, error) {
	...
	if os.Getenv("DIGINFRA_SPARSE_CHECKOUT") == "true" {
