...
import (
	...
	intSync "github.com/diginfra/diginfra/internal/sync"
	...
	c := &RunContext{
		...
		ContextValues: NewContextValues(
			map[string]interface{}{
				...
				"cliPlatform":           os.Getenv("DIGINFRA_CLI_PLATFORM"),
				"ciScript":              ciScript(),
				"ciPostCondition":       os.Getenv("DIGINFRA_CI_POST_CONDITION"),
				"ciPercentageThreshold": os.Getenv("DIGINFRA_CI_PERCENTAGE_THRESHOLD"),
			}),
		ModuleMutex: &intSync.KeyMutex{},
...
// SetIsDiginfraComment identifies that the primary command being run is `diginfra comment`
func (r *RunContext) SetIsDiginfraComment() {
	r.isCommentCmd = true
}

func (r *RunContext) IsDiginfraComment() bool {
...
func ciScript() string {
	if IsEnvPresent("DIGINFRA_CI_IMAGE") {
		return "ci-image"
	} else if IsEnvPresent("DIGINFRA_GITHUB_ACTION") {
		return "diginfra-github-action"
	} else if IsEnvPresent("DIGINFRA_CI_DIFF") {
		return "ci-diff"
	} else if IsEnvPresent("DIGINFRA_CI_ATLANTIS_DIFF") {
		return "ci-atlantis-diff"
	} else if IsEnvPresent("DIGINFRA_CI_JENKINS_DIFF") {
...
func ciPlatform() string {
	if os.Getenv("DIGINFRA_CI_PLATFORM") != "" {
		return os.Getenv("DIGINFRA_CI_PLATFORM")
	}
