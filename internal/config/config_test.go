...
`),
			error: &YamlError{
				base: "config file is invalid, see https://diginfra.khulnasoft.com/config-file for valid options",
				errors: []error{
					&YamlError{
...
`),
			error: &YamlError{
				base: "config file is invalid, see https://diginfra.khulnasoft.com/config-file for file specification",
				errors: []error{
	...
	for i, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// we need to remove DIGINFRA_TERRAFORM_CLOUD_TOKEN value for these tests.
			// as CI uses DIGINFRA_TERRAFORM_CLOUD_TOKEN for private registry tests. This means the expected value
			// will be inconsistent and show "***".
			key := "DIGINFRA_TERRAFORM_CLOUD_TOKEN"
			v := os.Getenv(key)
			os.Unsetenv(key)
...
func TestConfig_CachePath(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "should return the root directory with no .diginfra directory in either the root or parent directories",
			path:     "testdata/no_cache_init",
			expected: "testdata/no_cache_init",
		},
		{
			name:     "should return the parent directory with an diginfra cache in the parent directory",
			path:     "testdata/parent_cache_path/parent/child",
			expected: "testdata/parent_cache_path/parent",
		},
		{
			name:     "should return the root directory path with an diginfra cache in the root directory",
