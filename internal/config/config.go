...
type AutodetectConfig struct {
	...
	// MaxSearchDepth configures the number of folders that Diginfra should
...
// is outlined here: https://www.diginfra.khulnasoft.com/config-file
type Project struct {
	...
	// TerraformCloudToken sets the Team API Token or User API Token so diginfra can use it to access the plan.
	...
	// TerraformUseState sets if the users wants to use the terraform state for diginfra ops.
...
func DefaultConfig() *Config {
	return &Config{
		...
		DefaultPricingAPIEndpoint: "https://pricing.api.diginfra.khulnasoft.com",
		PricingAPIEndpoint:        "",
		DashboardAPIEndpoint:      "https://dashboard.api.diginfra.khulnasoft.com",
		DashboardEndpoint:         "https://infra-dashboard.khulnasoft.com",
...
// CachePath finds path which contains the .diginfra directory. It traverses parent directories until a .diginfra
// folder is found. If no .diginfra folders exist then CachePath uses the current wd.
...
func (c *Config) cachePath(dir string) string {
	for {
		cachePath := filepath.Join(dir, DiginfraDir)
...
func (c *Config) loadEnvVars() error {
	err := envconfig.Process("DIGINFRA", c)
	if err != nil {
		return err
	}

	for _, project := range c.Projects {
		err = envconfig.Process("DIGINFRA", project)
		if err != nil {
			return err
		}
	}
...
func IsTest() bool {
	return os.Getenv("DIGINFRA_ENV") == "test" || strings.HasSuffix(os.Args[0], ".test")
}

func IsDev() bool {
	return os.Getenv("DIGINFRA_ENV") == "dev"
}

func loadDotEnv() error {
	if os.Getenv("DIGINFRA_DISABLE_ENVFILE") == "true" {
		return nil
	}
