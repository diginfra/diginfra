...
var (
	StubMetadata = Metadata{
		Remote: urlStringToRemote("https://github.com/diginfra/diginfra"),
		Branch: Branch{
			Name: "stub-branch",
		},
		Commit: Commit{
...
func buildEnvProvidedMetadata() Metadata {
	remote := urlStringToRemote(getEnv("DIGINFRA_VCS_REPOSITORY_URL"))
	m := Metadata{
		Remote: remote,
		Branch: Branch{
			Name: getEnv("DIGINFRA_VCS_BRANCH"),
		},
		Commit:      buildDefaultCommit(),
		PullRequest: buildDefaultPR(),
		Pipeline:    &Pipeline{ID: getEnv("DIGINFRA_VCS_PIPELINE_RUN_ID")},
	}
...
func buildDefaultPR() *PullRequest {
	if !keysSet(
		"DIGINFRA_VCS_PULL_REQUEST_ID",
		"DIGINFRA_VCS_PROVIDER",
		"DIGINFRA_VCS_PULL_REQUEST_TITLE",
		"DIGINFRA_VCS_PULL_REQUEST_AUTHOR",
		"DIGINFRA_VCS_PULL_REQUEST_LABELS",
		"DIGINFRA_VCS_BRANCH",
		"DIGINFRA_VCS_BASE_BRANCH",
		"DIGINFRA_VCS_PULL_REQUEST_URL",
	) {
		return nil
	}

	provider := strings.ToLower(getEnv("DIGINFRA_VCS_PROVIDER"))
	if _, ok := allowedProviders[provider]; !ok && provider != "" {
		logging.Logger.Warn().Msgf("provided value for DIGINFRA_VCS_PROVIDER '%s' is not valid. Setting vcsProvider to an empty string", provider)
		provider = ""
	}

	prURL := getEnv("DIGINFRA_VCS_PULL_REQUEST_URL")
	prID := getEnv("DIGINFRA_VCS_PULL_REQUEST_ID")
	if prURL != "" && prID == "" {
	...
	return &PullRequest{
		...
		Title:        getEnv("DIGINFRA_VCS_PULL_REQUEST_TITLE"),
		Author:       getEnv("DIGINFRA_VCS_PULL_REQUEST_AUTHOR"),
		Labels:       getEnvList("DIGINFRA_VCS_PULL_REQUEST_LABELS"),
		SourceBranch: getEnv("DIGINFRA_VCS_BRANCH"),
		BaseBranch:   getEnv("DIGINFRA_VCS_BASE_BRANCH"),
		URL:          prURL,
	}
}
...
func buildDefaultCommit() Commit {
	return Commit{
		SHA:         getEnv("DIGINFRA_VCS_COMMIT_SHA"),
		AuthorName:  getEnv("DIGINFRA_VCS_COMMIT_AUTHOR_NAME"),
		AuthorEmail: getEnv("DIGINFRA_VCS_COMMIT_AUTHOR_EMAIL"),
		Time:        envToTime("DIGINFRA_VCS_COMMIT_TIMESTAMP"),
		Message:     getEnv("DIGINFRA_VCS_COMMIT_MESSAGE"),
	}
}
...
func envToTime(key string) time.Time {
	...
	if err != nil {
		logging.Logger.Debug().Err(err).Msgf("could not parse 'DIGINFRA_COMMIT_TIMESTAMP' value '%s' as int64 timestamp", env)
		return time.Time{}
	}
...
func (f *metadataFetcher) isTest() bool {
	...
	return getEnv("DIGINFRA_ENV") == "test" || strings.HasSuffix(os.Args[0], ".test")
}
...
func (f *metadataFetcher) Get(path string, gitDiffTarget *string) (m Metadata, err error) {
	...
	if f.isTest() {
		logging.Logger.Debug().Msg("returning stub metadata as Diginfra is running in test mode")
...
func (f *metadataFetcher) getGithubMetadata(path string, gitDiffTarget *string) (Metadata, error) {
	...
	if m.Branch.Name == "HEAD" {
		...
		clonePath := fmt.Sprintf("/tmp/diginfra-%s-%s", gjson.GetBytes(event, "repository.name").String(), headRef)
		unlock := f.mu.Lock(clonePath)

		// if the clone path already exists then let's just do a plain open. We might hit this
		// if the user is running multiple Diginfra commands on the head commit. We don't want
		// to clone each time.
...
// This is used to aggregate Diginfra metadata across commands used in the same pipeline.
