...
import (
	...
	"github.com/diginfra/diginfra/internal/sync"
)
...
func Test_urlStringToRemote(t *testing.T) {
	tests := []struct {
		...
		{"git@bitbucket.org:8888/~test.diginfra.khulnasoft.com/diginfra-bitbucket-pipeline.git", Remote{
			Host: "bitbucket.org",
			Name: "~test.diginfra.khulnasoft.com/diginfra-bitbucket-pipeline",
			URL:  "https://bitbucket.org:8888/~test.diginfra.khulnasoft.com/diginfra-bitbucket-pipeline.git",
		}},
	}
...
func Test_metadataFetcher_GetLocalMetadataMergesWithEnv(t *testing.T) {
	...
	t.Setenv("GITHUB_ACTIONS", "")
	t.Setenv("DIGINFRA_VCS_COMMIT_AUTHOR_NAME", providedName)

	pullID := "1234"
	t.Setenv("DIGINFRA_VCS_PULL_REQUEST_ID", pullID)
...
func Test_metadataFetcher_Get_ReturnsUserDefinedEnvs(t *testing.T) {
	...
	t.Setenv("DIGINFRA_VCS_PULL_REQUEST_ID", "DIGINFRA_VCS_PULL_REQUEST_ID_VALUE")
	t.Setenv("DIGINFRA_VCS_PROVIDER", "github")
	t.Setenv("DIGINFRA_VCS_PULL_REQUEST_TITLE", "DIGINFRA_VCS_PULL_REQUEST_TITLE_VALUE")
	t.Setenv("DIGINFRA_VCS_PULL_REQUEST_AUTHOR", "DIGINFRA_VCS_PULL_REQUEST_AUTHOR_VALUE")
	t.Setenv("DIGINFRA_VCS_PULL_REQUEST_LABELS", "DIGINFRA_VCS_PULL_REQUEST_LABELS_VALUE_1, DIGINFRA_VCS_PULL_REQUEST_LABELS_VALUE_2")
	t.Setenv("DIGINFRA_VCS_BRANCH", "DIGINFRA_BRANCH_VALUE")
	t.Setenv("DIGINFRA_VCS_BASE_BRANCH", "DIGINFRA_VCS_BASE_BRANCH_VALUE")
	t.Setenv("DIGINFRA_VCS_PULL_REQUEST_URL", "https://github.com/diginfra/test-repo/pull/1979")
	t.Setenv("DIGINFRA_VCS_COMMIT_SHA", "DIGINFRA_COMMIT_VALUE")
	t.Setenv("DIGINFRA_VCS_COMMIT_AUTHOR_NAME", "DIGINFRA_COMMIT_AUTHOR_NAME_VALUE")
	t.Setenv("DIGINFRA_VCS_COMMIT_AUTHOR_EMAIL", "DIGINFRA_COMMIT_AUTHOR_EMAIL_VALUE")
	timestamp := fmt.Sprintf("%d", tt.Unix())
	t.Setenv("DIGINFRA_VCS_COMMIT_TIMESTAMP", timestamp)
	t.Setenv("DIGINFRA_VCS_COMMIT_MESSAGE", "DIGINFRA_COMMIT_MESSAGE_VALUE")
	t.Setenv("DIGINFRA_VCS_REPOSITORY_URL", "https://github.com/diginfra/test-repo.git")
	t.Setenv("DIGINFRA_VCS_BRANCH", "DIGINFRA_BRANCH_VALUE")
	t.Setenv("DIGINFRA_VCS_PIPELINE_RUN_ID", "DIGINFRA_VCS_PIPELINE_RUN_ID_VALUE")
	...
	assert.Equal(t, Metadata{
		Remote: Remote{
			Host: "github.com",
			Name: "diginfra/test-repo",
			URL:  "https://github.com/diginfra/test-repo.git",
		},
		Branch: Branch{
			Name: "DIGINFRA_BRANCH_VALUE",
		},
		Commit: Commit{
			SHA:         "DIGINFRA_COMMIT_VALUE",
			AuthorName:  "DIGINFRA_COMMIT_AUTHOR_NAME_VALUE",
			AuthorEmail: "DIGINFRA_COMMIT_AUTHOR_EMAIL_VALUE",
			Time:        tt,
			Message:     "DIGINFRA_COMMIT_MESSAGE_VALUE",
		},
		PullRequest: &PullRequest{
			ID:           "DIGINFRA_VCS_PULL_REQUEST_ID_VALUE",
			VCSProvider:  "github",
			Title:        "DIGINFRA_VCS_PULL_REQUEST_TITLE_VALUE",
			Author:       "DIGINFRA_VCS_PULL_REQUEST_AUTHOR_VALUE",
			Labels:       []string{"DIGINFRA_VCS_PULL_REQUEST_LABELS_VALUE_1", "DIGINFRA_VCS_PULL_REQUEST_LABELS_VALUE_2"},
			SourceBranch: "DIGINFRA_BRANCH_VALUE",
			BaseBranch:   "DIGINFRA_VCS_BASE_BRANCH_VALUE",
			URL:          "https://github.com/diginfra/test-repo/pull/1979",
		},
		Pipeline: &Pipeline{ID: "DIGINFRA_VCS_PIPELINE_RUN_ID_VALUE"},
	}, actual)
}
