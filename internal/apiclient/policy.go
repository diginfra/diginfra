...
// UploadPolicyData sends a filtered set of a project's resource information to Diginfra Cloud and
...
type policy2Resource struct {
	...
	Metadata                                policy2DiginfraMetadata `json:"diginfraMetadata"`
...
type policy2DiginfraMetadata struct {
	Calls          []policy2DiginfraMetadataCall `json:"calls"`
...
type policy2DiginfraMetadataCall struct {
	Filename  string `json:"filename"`
...
func filterResource(rd *schema.ResourceData, al allowList) policy2Resource {
	...
	var mdCalls []policy2DiginfraMetadataCall
	for _, c := range rd.Metadata["calls"].Array() {
		mdCalls = append(mdCalls, policy2DiginfraMetadataCall{
	...
	metadata := policy2DiginfraMetadata{
		Calls:          mdCalls,
		Checksum:       checksum,
