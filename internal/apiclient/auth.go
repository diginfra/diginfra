...
func (a AuthClient) Login(contextVals map[string]interface{}) (string, string, error) {
	...
	fmt.Printf("    %s\n", startURL)
	fmt.Println("\n- Or log in/sign up at https://infra-dashboard.khulnasoft.com, copy your API key\n    from Org Settings and run `diginfra configure set api_key MY_KEY`")
	fmt.Printf("\nWaiting...\n\n")
...
func (a AuthClient) startCallbackServer(listener net.Listener, generatedState string) (string, string, error) {
	...
	if resp.apiKey == "" || resp.err != nil {
		return "", "", fmt.Errorf("Authentication failed. Please get your API token from %s", ui.LinkString("https://infra-dashboard.khulnasoft.com"))
	}
