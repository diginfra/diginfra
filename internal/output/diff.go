...
func ToDiff(out Root, opts Options) ([]byte, error) {
	...
	if len(noDiffProjects) > 0 && opts.ShowSkipped {
		...
		if len(noDiffProjects) == 1 {
			...
			s += fmt.Sprintf("Run the following command to see its breakdown: %s", ui.PrimaryString("diginfra breakdown --path=/path/to/code"))
		} else {
			s += fmt.Sprintf("%d projects have no cost estimate changes.\n", len(noDiffProjects))
			s += fmt.Sprintf("Run the following command to see their breakdown: %s", ui.PrimaryString("diginfra breakdown --path=/path/to/code"))
		}
