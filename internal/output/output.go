...
var exampleProjectsRegex = regexp.MustCompile(`^diginfra\/(diginfra\/examples|example-terraform)\/`)
...
// a uniform "Policies" section of the diginfra comment.
...
// GovernanceFailures defines a list of governance failures that were returned from diginfra cloud.
...
func (r *Root) summaryMessage(showSkipped bool) string {
	...
	if r.Summary.TotalUnsupportedResources != nil && *r.Summary.TotalUnsupportedResources > 0 {
		...
		if showSkipped {
			msg += fmt.Sprintf(", see %s:", ui.SecondaryLinkString("https://diginfra.khulnasoft.com/requested-resources"))
