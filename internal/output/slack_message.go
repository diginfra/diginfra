...
func ToSlackMessage(out Root, opts Options) ([]byte, error) {
	...
	blocks := []slack.Block{
		slack.NewSectionBlock(
			&slack.TextBlockObject{
				...
				Text: fmt.Sprintf("💰 Diginfra estimate: *%s*", formatCostChangeSentence(out.Currency, out.PastTotalMonthlyCost, out.TotalMonthlyCost, true)),
			},
	...
	diffMsg := fmt.Sprintf("*Diginfra output*\n```%s```", ui.StripColor(string(diff)))
