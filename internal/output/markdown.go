...
var (
	usageFileDocsURL             = "https://www.diginfra.khulnasoft.com/docs/features/usage_based_resources/#diginfra-usageyml"
	usageDefaultsDocsURL         = "https://www.diginfra.khulnasoft.com/docs/features/usage_based_resources"
...
func cloudSettingsStr(out Root, useMarkdownLinks bool) string {
	if !useMarkdownLinks {
		return "Diginfra Cloud settings"
	}
	...
	return fmt.Sprintf("[Diginfra Cloud settings](%s)", usageDefaultsURL)
}
...
func handleUsageApiEnabledMessage(out Root, useMarkdownLinks bool) string {
	if out.Metadata.UsageFilePath != "" || out.Metadata.ConfigFileHasUsageFile {
		return fmt.Sprintf("*Usage costs were estimated by merging diginfra-usage.yml and %s.", cloudSettingsStr(out, useMarkdownLinks))
	}
...
func handleUsageApiDisabledMessage(out Root, useMarkdownLinks bool) string {
	if out.Metadata.UsageFilePath != "" || out.Metadata.ConfigFileHasUsageFile {
		return fmt.Sprintf("*Usage costs were estimated using diginfra-usage.yml, see %s for other options.", usageDocsStr(useMarkdownLinks))
	}
