...
// validates that the Diginfra JSON body is valid by checking the that the version attribute is within a supported range.
func Load(p string) (Root, error) {
	var out Root
	_, err := os.Stat(p)
	if errors.Is(err, os.ErrNotExist) {
		return out, errors.New("Diginfra JSON file does not exist, generate it by running the following command then try again:\ndiginfra breakdown --path /code --format json --out-file diginfra-base.json")
	}

	data, err := os.ReadFile(p)
	if err != nil {
		return out, fmt.Errorf("error reading Diginfra JSON file %w", err)
	}

	err = json.Unmarshal(data, &out)
	if err != nil {
		return out, fmt.Errorf("invalid Diginfra JSON file %w, generate it by running the following command then try again:\ndiginfra breakdown --path /code --format json --out-file diginfra-base.json", err)
	}

	if !checkOutputVersion(out.Version) {
		return out, fmt.Errorf("invalid Diginfra JSON file version. Supported versions are %s ≤ x ≤ %s", minOutputVersion, maxOutputVersion)
	}
...
func CompareTo(c *config.Config, current, prior Root) (Root, error) {
	...
	for _, p := range prior.Projects {
		if _, ok := priorProjects[p.LabelWithMetadata()]; ok {
			return Root{}, fmt.Errorf("Invalid --compare-to Diginfra JSON, found duplicate project name %s", p.LabelWithMetadata())
		}
...
func Combine(inputs []ReportInput) (Root, error) {
	...
	if invalidMetadata {
		return combined, clierror.NewWarningF(
			"combining Diginfra JSON for different VCS repositories %s. Using %s as the top-level repository in the outputted JSON",
...
func checkCurrency(inputCurrency, fileCurrency string) (string, error) {
	...
	if inputCurrency != fileCurrency {
		return "", fmt.Errorf("Invalid Diginfra JSON file currency mismatch.  Can't combine %s and %s", inputCurrency, fileCurrency)
	}
