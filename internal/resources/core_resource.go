...
func PopulateArgsWithUsage(args interface{}, u *schema.UsageData) {
	...
	for i := 0; i < s.NumField(); i++ {
		...
		// Get the diginfra tag data.
		diginfraTagSplitted := strings.Split(s.Type().Field(i).Tag.Get("diginfra_usage"), ",")
		if len(diginfraTagSplitted) < 1 {
			continue
		}
		// Key name for the usage file
		usageKey := diginfraTagSplitted[0]
		if usageKey == "" {
			continue
		}
