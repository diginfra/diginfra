...
import (
	...
	"github.com/diginfra/diginfra/internal/usage/aws"
)
...
type Instance struct {
	...
	// "usage" args
	OperatingSystem               *string  `diginfra_usage:"operating_system"`
	ReservedInstanceType          *string  `diginfra_usage:"reserved_instance_type"`
	ReservedInstanceTerm          *string  `diginfra_usage:"reserved_instance_term"`
	ReservedInstancePaymentOption *string  `diginfra_usage:"reserved_instance_payment_option"`
	MonthlyCPUCreditHours         *int64   `diginfra_usage:"monthly_cpu_credit_hrs"`
	VCPUCount                     *int64   `diginfra_usage:"vcpu_count"`
	MonthlyHours                  *float64 `diginfra_usage:"monthly_hrs"`
}
...
func (a *Instance) BuildResource() *schema.Resource {
	if strings.ToLower(a.Tenancy) == "host" {
		if a.HasHost {
			...
			logging.Logger.Warn().Msgf("Skipping resource %s. Diginfra currently does not support host tenancy for AWS EC2 instances without Host ID set up", a.Address)
			return nil
		}
