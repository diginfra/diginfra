# Adding a new resource to Diginfra

This guide will help you master the art of adding cloud resources to Diginfra. It might seem long but it's pretty easy to follow, we've even built tooling to auto-generate the required scaffolding and there are integration tests too!
...
We're going to explain how Diginfra manages cloud resources as that helps with adding support for new resources.
...
- [Update Diginfra docs](#update-diginfra-docs)
...
- A working development environment for the Diginfra CLI. You can set it up using [this guide](/CONTRIBUTING.md#setting-up-the-development-environment).
- Access to the Cloud Pricing API database via PostgreSQL. Contact us in the [Slack channel](https://www.diginfra.khulnasoft.com/community) if you need access.
...
We are going to add resources for `AWS Transfer Family` service. The [GitHub issue](https://github.com/diginfra/diginfra/issues/1030) includes:
...
Diginfra CLI has a dedicated command that makes generating file structure for new resources quick and easy.
...
and running the following command to generate initial Diginfra output:
...
Diginfra CLI has a test suite. Each resource also has tests. We will run them to ensure that the code we're adding is working correctly.
...
```
DIGINFRA_LOG_LEVEL=warn go test -timeout 30m -ldflags="-X 'github.com/diginfra/diginfra/internal/version.Version=v0.9.14+13-gec7fae1b-dirty'" ./internal/providers/terraform/aws --run TestTransferServer -v -update
time="2021-12-02T10:51:17+01:00" level=info msg="Running command: /Users/diginfra/.asdf/shims/terraform init -no-color"
...
PASS
ok      github.com/diginfra/diginfra/internal/providers/terraform/aws       7.130s
```
...
∙ 1 was estimated, 1 includes usage-based costs, see https://diginfra.khulnasoft.com/usage-file
```
...
The first three components in the [GitHub issue](https://github.com/diginfra/diginfra/issues/1030) are `FTP protocol enabled`, `SFTP protocol enabled` and `FTPS protocol enabled`. They all rely on the `protocols` Terraform attribute, so they probably will use the same cost component. We can start with them.
...
// It uses the `diginfra_usage` struct tags to populate data into the TransferServer.
...
FAIL
FAIL    github.com/diginfra/diginfra/internal/providers/terraform/aws 3.335s
FAIL
...
Diginfra stores over 3M prices for multiple services, and each cloud provider may have a different structure for storing such data. JSON is a great format to keep such information. However, it might be a bit overwhelming to search through.
...
+∙ 2 were estimated, 2 include usage-based costs, see https://diginfra.khulnasoft.com/usage-file
```
...
Diginfra supports passing usage data in through a usage YAML file. We'll add all necessary parts for the cost components first, and then we'll see how they use the YAML data.
...
type TransferServer struct {
	...
	// "usage" args
	MonthlyDataDownloadedGB *float64 `diginfra_usage:"monthly_data_downloaded_gb"`
	MonthlyDataUploadedGB   *float64 `diginfra_usage:"monthly_data_uploaded_gb"`
}
```

We expect the values to be floats. Here, we map the attributes to their counterparts in the usage file as `diginfra_usage:"<attribute name>"`.
...
> Note: As the YAML values are floats and `MonthlyQuantity` expects a decimal pointer, we have to make a type conversion. Diginfra code has several helper functions like `floatPtrToDecimalPtr` to make it easier.
...
PASS
ok      github.com/diginfra/diginfra/internal/providers/terraform/aws 4.330s
```
...
The last thing is to update `diginfra-usage-example.yml` with an example of usage parameters so Diginfra users can see how they can use them. Open the file and add the following block:
...
The Diginfra convention is to place it in alphabetical order for easier navigation, use `my_<resource_name>` as the resource's name, and provide a comment with parameter description.
...
[Diginfra documentation](https://www.diginfra.khulnasoft.com/docs/supported_resources/overview/) lists all supported paid and free resources. As we add a new resource, let's also update the docs.

1. Create a fork of [docs repo](https://github.com/diginfra/docs).
1. Create a new branch and update [this file](https://github.com/diginfra/docs/blob/master/docs/supported_resources/aws.md):
...
1. Commit your changes with the `docs:` prefix, for example, `docs: add AWS Transfer Family resources` message. Push it to GitHub and create a pull request to Diginfra's docs repository.

## Prepare a Pull Request for review

Last step is to commit our changes and create a pull request in [Diginfra repo](https://github.com/diginfra/diginfra).

1. Commit your changes using `feat(aws):` prefix (Diginfra team uses [Conventional Commits](https://www.conventionalcommits.org/en/v1.0.0/)).
1. Push the branch to your fork and create a new pull request in the Diginfra repo using the following template:
...
- [ ] Added usage parameters to diginfra-usage-example.yml
...
Mark all the checkboxes you've done and wait for somebody from the Diginfra team to review your changes.

🎉  Congratulations! You've added your first Diginfra resource. Great job, thank you! Every new resource makes Diginfra more useful and gets users closer to loving their cloud bill! 💰📉

<p align="center">
  <img src="https://diginfra-public-dumps.s3.amazonaws.com/images/congratulations.gif" alt="Congratulations!" />
</p>
...
- unsupported resources: if there are certain conditions that can be checked inside a resource Go file, which means that the resource is not yet supported, log a warning to explain what is not supported and return a `nil` response as shown below.
  ...
  ```go
	if d.Get("placement_tenancy").String() == "host" {
		logging.Logger.Warn().Msgf("Skipping resource %s. Diginfra currently does not support host tenancy for AWS Launch Configurations", d.Address)
		return nil
	}
  ```
...
type MyResource struct {
	// ...
	MonthlyDataProcessedGB *RegionsUsage `diginfra_usage:"monthly_processed_gb"`
}
```
...
type RegionsUsage struct {
	USGovWest1   *float64 `diginfra_usage:"us_gov_west_1"`
	// Add your new region here with the diginfra_usage struct tag
...
```go
type RegionsUsage struct {
	AsiaEast1              *float64 `diginfra_usage:"asia_east1"`
	// Add your new region here with the diginfra_usage struct tag
