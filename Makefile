BINARY := diginfra
PKG := github.com/diginfra/diginfra/cmd/diginfra
VERSION := $(shell scripts/get_version.sh HEAD $(NO_DIRTY))
LD_FLAGS := -ldflags="-X 'github.com/diginfra/diginfra/internal/version.Version=$(VERSION)'"
BUILD_FLAGS := $(LD_FLAGS) -v

DEV_ENV := dev
ifdef DIGINFRA_ENV
	DEV_ENV := $(DIGINFRA_ENV)
endif
...
run:
	env DIGINFRA_ENV=$(DEV_ENV) go run $(LD_FLAGS) $(PKG) $(ARGS)

jsonschema:
	go run ./cmd/jsonschema/main.go --out-file ./schema/diginfra.schema.json
...
test:
	DIGINFRA_LOG_LEVEL=warn go test -short $(LD_FLAGS) ./... $(or $(ARGS), -cover)

# Run only short unit tests with parallelism disabled and verbosity true.
# This is useful for pinpointing a hanging test.
test_verbose:
	DIGINFRA_LOG_LEVEL=warn go test -p 1 -v -short $(LD_FLAGS) ./... $(or $(ARGS), -cover)

# Run all tests
test_all:
	DIGINFRA_LOG_LEVEL=warn go test -timeout 30m $(LD_FLAGS) ./... $(or $(ARGS), -cover)

# Run unit tests and shared integration tests
test_shared_int:
	DIGINFRA_LOG_LEVEL=warn go test -timeout 30m $(LD_FLAGS) \
...
test_cmd:
	DIGINFRA_LOG_LEVEL=warn go test -timeout 30m $(LD_FLAGS) ./cmd/diginfra $(or $(ARGS), -cover)

test_update_cmd:
	DIGINFRA_LOG_LEVEL=warn go test -timeout 30m $(LD_FLAGS) ./cmd/diginfra $(or $(ARGS), -update -cover)

test_usage:
	DIGINFRA_LOG_LEVEL=warn go test -timeout 30m $(LD_FLAGS) ./internal/usage $(or $(ARGS), -cover)

test_update_usage:
	DIGINFRA_LOG_LEVEL=warn go test -timeout 30m $(LD_FLAGS) ./internal/usage $(or $(ARGS), -update -cover)

# Run AWS resource tests
test_aws:
	DIGINFRA_LOG_LEVEL=warn go test -timeout 30m $(LD_FLAGS) ./internal/providers/terraform/aws $(or $(ARGS), -cover)

# Run Google resource tests
test_google:
	DIGINFRA_LOG_LEVEL=warn go test -timeout 30m $(LD_FLAGS) ./internal/providers/terraform/google $(or $(ARGS), -cover)

# Run Azure resource tests
test_azure:
	DIGINFRA_LOG_LEVEL=warn go test -timeout 30m $(LD_FLAGS) ./internal/providers/terraform/azure $(or $(ARGS), -cover)

# Update AWS golden files tests
test_update:
	DIGINFRA_LOG_LEVEL=warn go test -timeout 30m $(LD_FLAGS) ./internal/providers/... $(or $(ARGS), -update -cover)

# Update golden files tests
test_update_aws:
	DIGINFRA_LOG_LEVEL=warn go test -timeout 30m $(LD_FLAGS) ./internal/providers/terraform/aws $(or $(ARGS), -update -cover)

# Update Google golden files tests
test_update_google:
	DIGINFRA_LOG_LEVEL=warn go test -timeout 30m $(LD_FLAGS) ./internal/providers/terraform/google $(or $(ARGS), -update -cover)

# Update Azure golden files tests
test_update_azure:
	DIGINFRA_LOG_LEVEL=warn go test -timeout 30m $(LD_FLAGS) ./internal/providers/terraform/azure $(or $(ARGS), -update -cover)
...
parser_benchmarks:
	go test -bench=. -benchmem -cpuprofile=cpu.out -memprofile=mem.out -trace=trace.out github.com/diginfra/diginfra/internal/hcl
