...
import (
	...
	main "github.com/diginfra/diginfra/cmd/diginfra"
...
var (
	...
	urlRegex         = regexp.MustCompile(`https://infra-dashboard.khulnasoft.com/share/.*`)
	projectPathRegex = regexp.MustCompile(`(Project:) .*/(examples|cmd/diginfra)/(.*)`)
	versionRegex     = regexp.MustCompile(`Diginfra (v|preview).*`)
...
func GetCommandOutput(t *testing.T, args []string, testOptions *GoldenFileOptions, ctxOptions ...func(ctx *config.RunContext)) []byte {
	...
	os.Setenv("DIGINFRA_VCS_REPOSITORY_URL", "https://github.com/diginfra/diginfra")
	os.Setenv("DIGINFRA_VCS_PULL_REQUEST_URL", "NOT_APPLICABLE")
...
func stripDynamicValues(actual []byte) []byte {
	...
	actual = urlRegex.ReplaceAll(actual, []byte("https://infra-dashboard.khulnasoft.com/share/REPLACED_SHARE_CODE"))
	actual = projectPathRegex.ReplaceAll(actual, []byte("$1 REPLACED_PROJECT_PATH/$3"))
	actual = versionRegex.ReplaceAll(actual, []byte("Diginfra vREPLACED_VERSION"))
