...
func outputCmd(ctx *config.RunContext) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "output",
		Short: "Combine and output Diginfra JSON files in different formats",
		Long:  "Combine and output Diginfra JSON files in different formats",
		Example: `  Show a breakdown from multiple Diginfra JSON files:

      diginfra output --path out1.json --path out2.json --path out3.json

  Create HTML report from multiple Diginfra JSON files:

      diginfra output --format html --path "out*.json" --out-file output.html # glob needs quotes

  Merge multiple Diginfra JSON files:

      diginfra output --format json --path "out*.json" # glob needs quotes

  Create markdown report to post in a GitHub comment:

      diginfra output --format github-comment --path "out*.json" # glob needs quotes

  Create markdown report to post in a GitLab comment:

      diginfra output --format gitlab-comment --path "out*.json" # glob needs quotes

  Create markdown report to post in a Azure DevOps Repos comment:

      diginfra output --format azure-repos-comment --path "out*.json" # glob needs quotes

  Create markdown report to post in a Bitbucket comment:

      diginfra output --format bitbucket-comment --path "out*.json" # glob needs quotes`,
		ValidArgs: []string{"--", "-"},
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			combined.Metadata.DiginfraCommand = "output"
			...
			err = pricingClient.AddEvent("diginfra-output", ctx.EventEnv())
			if err != nil {
	...
	cmd.Flags().StringArrayP("path", "p", []string{}, "Path to Diginfra JSON files, glob patterns need quotes")
