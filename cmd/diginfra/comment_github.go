...
import (
	...
	"github.com/diginfra/diginfra/internal/comment"
...
func commentGitHubCmd(ctx *config.RunContext) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "github",
		Short: "Post an Diginfra comment to GitHub",
		Long:  "Post an Diginfra comment to GitHub",
		Example: `  Update comment on a pull request:

      diginfra comment github --repo my-org/my-repo --pull-request 3 --path diginfra.json --github-token $GITHUB_TOKEN

  Post a new comment to a commit:

      diginfra comment github --repo my-org/my-repo --commit 2ca7182 --path diginfra.json --behavior hide-and-new --github-token $GITHUB_TOKEN`,
		ValidArgs: []string{"--", "-"},
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !dryRun {
				...
				err = pricingClient.AddEvent("diginfra-comment", ctx.EventEnv())
				if err != nil {
					logging.Logger.Err(err).Msg("could not report diginfra-comment event")
				}
  ...
  delete-and-new    Delete previous matching comments and create a new comment`)
	...
	cmd.Flags().String("tag", "", "Customize hidden markdown tag used to detect comments posted by Diginfra")
