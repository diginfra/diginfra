...
func configureSetCmd(ctx *config.RunContext) *cobra.Command {
	cmd := &cobra.Command{
		...
		Example: `  Set your Diginfra API key:

      diginfra configure set api_key MY_API_KEY

  Set your Cloud Pricing API endpoint:

      diginfra	configure set pricing_api_endpoint https://cloud-pricing-api

  Set your preferred currency code (ISO 4217):

      diginfra	configure set currency EUR
`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			case "enable_dashboard":
				...
				if b && ctx.Config.IsSelfHosted() {
					return errors.New("The dashboard is part of Diginfra's hosted services. Contact hello@diginfra.khulnasoft.com for help")
				}
			...
			case "enable_cloud":
				...
				if b && ctx.Config.IsSelfHosted() {
					return errors.New("Diginfra Cloud is part of Diginfra's hosted services. Contact hello@diginfra.khulnasoft.com for help")
				}
...
func configureGetCmd(ctx *config.RunContext) *cobra.Command {
	cmd := &cobra.Command{
		...
		Example: `  Get your saved Diginfra API key:

      diginfra configure get api_key

  Get your saved Cloud Pricing API endpoint:

      diginfra	configure get pricing_api_endpoint

  Get your preferred currency:

      diginfra	configure get currency
`,
		Args: func(cmd *cobra.Command, args []string) error {
			if len(args) > 1 {
				return errors.New("Too many arguments")
			...
			case "pricing_api_endpoint":
				...
				if value == "" {
					msg := fmt.Sprintf("No Cloud Pricing API endpoint in your saved config (%s).\nSet an API key using %s.",
						...
						ui.PrimaryString("diginfra configure set pricing_api_endpoint https://cloud-pricing-api"),
					)
			...
			case "api_key":
				...
				if value == "" {
					msg := fmt.Sprintf("No API key in your saved config (%s).\nSet an API key using %s.",
						config.CredentialsFilePath(),
						ui.PrimaryString("diginfra configure set api_key MY_API_KEY"),
					)
					logging.Logger.Warn().Msg(msg)
				}
			case "currency":
				...
				if value == "" {
					msg := fmt.Sprintf("No currency in your saved config (%s), defaulting to USD.\nSet a currency using %s.",
						...
						ui.PrimaryString("diginfra configure set currency CURRENCY"),
					)
					logging.Logger.Warn().Msg(msg)
				}
			case "tls_insecure_skip_verify":
				...
				if value == "" {
					msg := fmt.Sprintf("Skipping TLS verification is not set in your saved config (%s).\nSet it using %s.",
						config.ConfigurationFilePath(),
						ui.PrimaryString("diginfra configure set tls_insecure_skip_verify true"),
					)
					logging.Logger.Warn().Msg(msg)
				}
			case "tls_ca_cert_file":
				...
				if value == "" {
					msg := fmt.Sprintf("No CA cert file in your saved config (%s).\nSet a CA certificate using %s.",
						config.ConfigurationFilePath(),
						ui.PrimaryString("diginfra configure set tls_ca_cert_file /path/to/ca.crt"),
					)
					logging.Logger.Warn().Msg(msg)
				}
			case "enable_dashboard":
