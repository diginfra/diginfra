...
import (
	...
	"github.com/diginfra/diginfra/internal/config/template"
...
func newGenerateConfigCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "config",
		Short: "Generate Diginfra config file from a template file",
		Long: `Generate Diginfra config file from a template file. See docs for template examples and syntax:

  https://www.diginfra.khulnasoft.com/docs/features/config_file/#template-syntax`,
		Example: `
      diginfra generate config --repo-path . --template-path diginfra.yml.tmpl --out-file diginfra.yml
      `,
		ValidArgs: []string{"--", "-"},
		RunE:      gen.run,
	}
	...
	cmd.Flags().StringVar(&gen.template, "template", "", "Diginfra template string that will generate the config-file yaml output")
	cmd.Flags().StringVar(&gen.templatePath, "template-path", "", "Path to the Diginfra template file that will generate the config-file yaml output")
...
func newGenerateCommand() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "generate",
		Short: "Generate configuration to help run Diginfra",
		Long:  "Generate configuration to help run Diginfra",
		Example: ` Generate Diginfra config file from a template file:

      diginfra generate config --repo-path . --template-path diginfra.yml.tmpl
      `,
		ValidArgs: []string{"--", "-"},
