...
func TestGenerateConfig(t *testing.T) {
	...
	GoldenFileCommandTest(
		...
		[]string{
			...
			"config",
			"--template-path",
			path.Join("./testdata", dir, "diginfra.yml.tmpl"),
			"--repo-path",
...
// in a tmp directory. It will then run the `diginfra generate config` command
...
func TestGenerateConfigAutoDetect(t *testing.T) {
	...
	for _, dir := range dirs {
		...
		t.Run(name, func(tt *testing.T) {
			...
			templatePath := path.Join(dir, "diginfra.yml.tmpl")
