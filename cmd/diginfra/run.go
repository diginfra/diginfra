...
import (
	...
	"github.com/diginfra/diginfra/internal/vcs"
	...
	"github.com/diginfra/diginfra/internal/prices"
	"github.com/diginfra/diginfra/internal/providers"
...
func addRunFlags(cmd *cobra.Command) {
	...
	cmd.Flags().String("config-file", "", "Path to Diginfra config file. Cannot be used with path, terraform* or usage-file flags")
	cmd.Flags().String("usage-file", "", "Path to Diginfra usage file that specifies values for usage-based resources")
...
func runMain(cmd *cobra.Command, runCtx *config.RunContext) error {
	...
	if runCtx.IsCloudUploadExplicitlyEnabled() {
		...
		if err != nil {
			logging.Logger.Err(err).Msg("Failed to upload to Diginfra Cloud")
		}

		r.RunID, r.ShareURL, r.CloudURL = result.RunID, result.ShareURL, result.CloudURL
	} else {
		logging.Logger.Debug().Msg("Skipping sending project results since Diginfra Cloud upload is not enabled.")
	}
	...
	err = pricingClient.AddEvent("diginfra-run", env)
	if err != nil {
...
func (r *parallelRunner) run() ([]projectResult, error) {
	...
	for _, p := range r.runCtx.Config.Projects {
		...
		if err != nil {
			m := fmt.Sprintf("%s\n\n", err)
			m += fmt.Sprintf("  Try adding a config-file to configure how Diginfra should run. See %s for details and examples.", ui.LinkString("https://diginfra.khulnasoft.com/config-file"))
...
func (r *parallelRunner) runProvider(job projectJob) (out *projectOutput, err error) {
	...
	if r.cmd.Name() == "diff" && job.provider.Type() == "terraform_state_json" {
		m := "Cannot use Terraform state JSON with the diginfra diff command.\n\n"
		m += fmt.Sprintf("Use the %s flag to specify the path to one of the following:\n", ui.PrimaryString("--path"))
		m += fmt.Sprintf(" - Terraform/Terragrunt directory\n - Terraform plan JSON file, see %s for how to generate this.", ui.SecondaryLinkString("https://diginfra.khulnasoft.com/troubleshoot"))
		return nil, clierror.NewCLIError(errors.New(m), "Cannot use Terraform state JSON with the diginfra diff command")
	}
...
func (r *parallelRunner) uploadCloudResourceIDs(projects []*schema.Project) error {
	...
	logging.Logger.Debug().Msg("Sending resource IDs to Diginfra Cloud for usage estimates")
...
func (r *parallelRunner) fetchProjectUsage(projects []*schema.Project) map[*schema.Project]schema.UsageMap {
	...
	logging.Logger.Debug().Msgf("Retrieving usage defaults for %s from Diginfra Cloud", resourceStr)
...
func (r *parallelRunner) populateActualCosts(projects []*schema.Project) {
	if r.runCtx.Config.UsageAPIEndpoint != "" {
		logging.Logger.Debug().Msg("Retrieving actual costs from Diginfra Cloud")
...
func loadRunFlags(cfg *config.Config, cmd *cobra.Command) error {
	...
	if cmd.Name() != "diginfra" && !hasPathFlag && !hasConfigFile {
		m := fmt.Sprintf("No path specified\n\nUse the %s flag to specify the path to one of the following:\n", ui.PrimaryString("--path"))
		m += fmt.Sprintf(" - Terraform/Terragrunt directory\n - Terraform plan JSON file, see %s for how to generate this.", ui.SecondaryLinkString("https://diginfra.khulnasoft.com/troubleshoot"))
		m += fmt.Sprintf("\n\nAlternatively, use --config-file to process multiple projects, see %s", ui.SecondaryLinkString("https://diginfra.khulnasoft.com/config-file"))
	...
	return func(cmd *cobra.Command, args []string) error {
		if ctx.Config.APIKey == "" {
			return fmt.Errorf("%s %s %s %s %s\n%s %s.\n%s %s %s",
				...
				"See",
				ui.LinkString("https://diginfra.khulnasoft.com/support"),
				"if you continue having issues.")
		}
