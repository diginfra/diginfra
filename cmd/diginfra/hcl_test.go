...
func TestHCLMultiProjectInfra(t *testing.T) {
	...
	t.Run("rel path", func(t *testing.T) {
		GoldenFileCommandTest(t, name,
			[]string{"breakdown", "--config-file", path.Join("./testdata", name, "diginfra.config.yml")},
			nil)
	})

	t.Run("abs path", func(t *testing.T) {
		abs, err := filepath.Abs(path.Join("./testdata", name, "diginfra.config.yml"))
...
func TestHCLMultiWorkspace(t *testing.T) {
	GoldenFileCommandTest(t, testutil.CalcGoldenFileTestdataDirName(),
		[]string{"breakdown", "--config-file", path.Join("./testdata", testutil.CalcGoldenFileTestdataDirName(), "diginfra.config.yml")},
		nil)
}
