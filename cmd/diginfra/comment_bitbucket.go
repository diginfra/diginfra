...
func commentBitbucketCmd(ctx *config.RunContext) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "bitbucket",
		Short: "Post an Diginfra comment to Bitbucket",
		Long:  "Post an Diginfra comment to Bitbucket",
  ...
  delete-and-new    Delete previous matching comments and create a new comment`)
	...
	cmd.Flags().String("tag", "", "Customize special text used to detect comments posted by Diginfra (placed at the bottom of a comment)")
