...
func TestDiffProjectName(t *testing.T) {
	...
	GoldenFileCommandTest(t, testName,
		[]string{
			"diff",
			"--config-file", path.Join("./testdata", testName, "diginfra-config.yml"),
...
func TestDiffWithDiginfraJSON(t *testing.T) {
	dir := path.Join("./testdata", testutil.CalcGoldenFileTestdataDirName())
	GoldenFileCommandTest(
		t,
		testutil.CalcGoldenFileTestdataDirName(),
...
projects:
  ...
  - path: %s`,
	...
	configFilePath := path.Join(dir, "diginfra.yml")
...
func TestDiffTerragruntSyntaxError(t *testing.T) {
	...
	GoldenFileCommandTest(
		...
		[]string{
			...
			"--config-file", filepath.Join(dir, "diginfra.config.yml"),
		},
		&GoldenFileOptions{},
	)
}
