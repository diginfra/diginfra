...
func diffCmd(ctx *config.RunContext) *cobra.Command {
  ...
  Use Terraform plan JSON:
	...
	cmd.Flags().String("compare-to", "", "Path to Diginfra JSON file to compare against")
...
func runDiff(cmd *cobra.Command, ctx *config.RunContext) error {
	if len(ctx.Config.Projects) > 0 {
		...
		// if the path provided is an Diginfra JSON we need to run a compare run
		current, err := output.Load(path)
		if err == nil {
			if ctx.Config.CompareTo == "" {
				return errors.New("Passing an Diginfra JSON as a --path argument is only valid using the --compare-to flag")
			}
...
func runCompare(cmd *cobra.Command, ctx *config.RunContext, current output.Root) error {
	...
	err = pricingClient.AddEvent("diginfra-run", ctx.EventEnv())
	if err != nil {
		logging.Logger.Err(err).Msg("could not report diginfra-run event")
	}
...
func checkDiffConfig(cfg *config.Config) error {
	for _, projectConfig := range cfg.Projects {
		if projectConfig.TerraformUseState {
			return errors.New("terraform_use_state cannot be used with `diginfra diff` as the Terraform state only contains the current state")
		}
  ...
  3. Generate a cost estimate diff: %s`,
				fmt.Sprintf("`diginfra breakdown --path %s --format json --out-file diginfra-base.json`", examplePath),
				fmt.Sprintf("`diginfra diff --path %s --compare-to diginfra-base.json`", examplePath),
			)
