...
func fetchExistingRelease(cli *github.Client, tag string) (*github.RepositoryRelease, error) {
	release, _, err := cli.Repositories.GetReleaseByTag(context.Background(), "diginfra", "diginfra", tag)
	if err != nil {
	...
	if newGitSha != "" {
		_, _, err = cli.Git.UpdateRef(context.Background(), "diginfra", "diginfra", &github.Reference{
	...
	for _, asset := range release.Assets {
		_, err = cli.Repositories.DeleteReleaseAsset(context.Background(), "diginfra", "diginfra", asset.GetID())
		if err != nil {
