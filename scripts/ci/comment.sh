...
#
# Please follow our guide: https://www.diginfra.khulnasoft.com/docs/guides/gitlab_ci_migration/ to migrate
# to our new GitHub Actions integration: https://gitlab.com/diginfra/diginfra-gitlab-ci/
#
...
echo "Please visit https://www.diginfra.khulnasoft.com/docs/guides/gitlab_ci_migration/ for instructions on how to upgrade."
echo "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
...
# Build flags for Diginfra to handle multiple paths
if [ "$#" -eq 0 ]; then
  echo "Expecting at least one Diginfra JSON file as argument"
  exit 1
fi
...
# Run diginfra output to generate the comment
export DIGINFRA_CI_POST_CONDITION=$behavior
body_file=$(mktemp)
# shellcheck disable=SC2086
diginfra_cmd="diginfra output $input_path_flags --format $format --show-skipped --out-file $body_file"
echo "Running diginfra output:" >&2
echo "  $diginfra_cmd" >&2
eval "$diginfra_cmd"
