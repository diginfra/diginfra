...
#
# Please visit : https://www.diginfra.khulnasoft.com/docs/integrations/cicd/ to migrate to
...
echo "Please visit https://github.com/diginfra/diginfra/blob/master/scripts/ci/README.md for instructions on how to upgrade."
echo "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
...
process_args () {
  ...
  DIGINFRA_BINARY=${DIGINFRA_BINARY:-diginfra}
  GITHUB_API_URL=${GITHUB_API_URL:-https://api.github.com}

  # Export as it's used by diginfra, not this script
  export DIGINFRA_LOG_LEVEL=${DIGINFRA_LOG_LEVEL:-info}
  export DIGINFRA_CI_DIFF=true
  export DIGINFRA_CI_POST_CONDITION=$post_condition
  export DIGINFRA_CI_PERCENTAGE_THRESHOLD=$percentage_threshold
...
build_breakdown_cmd () {
  breakdown_cmd="$DIGINFRA_BINARY breakdown --no-color --format json"
...
build_output_cmd () {
  output_cmd="$DIGINFRA_BINARY output --no-color --format diff --path $1"
...
build_msg () {
  ...
  if [ "$include_html" = true ]; then
    ...
    for (( i = 0; i < project_count; i++ )); do
      diff_resources=$(jq '.projects['"$i"'].diff.resources[]' diginfra_breakdown.json)
      if  [ -n "$diff_resources" ] || [ $project_count -eq 1 ]; then
        msg+="$(build_project_row "$i")"
      else
        if [ -z "$skipped_projects" ]; then
          skipped_projects="$(jq -r '.projects['"$i"'].name' diginfra_breakdown.json)"
        else
          skipped_projects="$skipped_projects, $(jq -r '.projects['"$i"'].name' diginfra_breakdown.json)"
        fi
      fi
    done
    ...
    msg+="<details>\n"
    msg+="  <summary><strong>Diginfra output</strong></summary>\n"
  else
  ...
  if [ "$include_html" = true ]; then
    ...
    msg+="<sub>\n"
    msg+="  Is this comment useful? <a href=\"https://infra-dashboard.khulnasoft.com/feedback/redirect?value=yes\" rel=\"noopener noreferrer\" target=\"_blank\">Yes</a>, <a href=\"https://infra-dashboard.khulnasoft.com/feedback/redirect?value=no\" rel=\"noopener noreferrer\" target=\"_blank\">No</a>, <a href=\"https://infra-dashboard.khulnasoft.com/feedback/redirect?value=other\" rel=\"noopener noreferrer\" target=\"_blank\">Other</a>\n"
    msg+="</sub>\n"
  fi
...
build_project_row () {
  ...
  max_name_length=64
  name=$(jq -r '.projects['"$i"'].name' diginfra_breakdown.json)
  ...
  past_monthly_cost=$(jq -r '.projects['"$i"'].pastBreakdown.totalMonthlyCost' diginfra_breakdown.json)
  monthly_cost=$(jq -r '.projects['"$i"'].breakdown.totalMonthlyCost' diginfra_breakdown.json)
  diff_monthly_cost=$(jq -r '.projects['"$i"'].diff.totalMonthlyCost' diginfra_breakdown.json)
...
post_to_azure_devops () {
  if [ "$BUILD_REASON" = "PullRequest" ]; then
    if [ "$BUILD_REPOSITORY_PROVIDER" = "GitHub" ]; then
      ...
      echo "Posting comments to Azure DevOps $BUILD_REPOSITORY_PROVIDER is not supported, email hello@diginfra.khulnasoft.com for help"
    fi
  else
    echo "Posting comment to Azure DevOps $BUILD_REASON is not supported, email hello@diginfra.khulnasoft.com for help"
  fi
}
...
cleanup () {
  rm -f diginfra_breakdown.json diginfra_breakdown_cmd diginfra_output_cmd
}
...
diginfra_breakdown_cmd=$(build_breakdown_cmd)
echo "$diginfra_breakdown_cmd" > diginfra_breakdown_cmd

echo "Running diginfra breakdown using:"
echo "  $ $(cat diginfra_breakdown_cmd)"
breakdown_output=$(cat diginfra_breakdown_cmd | sh)
echo "$breakdown_output" > diginfra_breakdown.json

diginfra_output_cmd=$(build_output_cmd "diginfra_breakdown.json")
echo "$diginfra_output_cmd" > diginfra_output_cmd

echo "Running diginfra output using:"
echo "  $ $(cat diginfra_output_cmd)"
diff_output=$(cat diginfra_output_cmd | sh)

project_count=$(jq -r '.projects | length' diginfra_breakdown.json)
past_total_monthly_cost=$(jq '(.pastTotalMonthlyCost // 0) | tonumber' diginfra_breakdown.json)
total_monthly_cost=$(jq '(.totalMonthlyCost // 0) | tonumber' diginfra_breakdown.json)
diff_total_monthly_cost=$(jq '(.diffTotalMonthlyCost // 0) | tonumber' diginfra_breakdown.json)
currency=$(jq -r '.currency | select (.!=null)' diginfra_breakdown.json)
...
diff_resources=$(jq '[.projects[].diff.resources[]] | add' diginfra_breakdown.json)
